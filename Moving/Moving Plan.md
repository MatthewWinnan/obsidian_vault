# Moving Plan

Moving date: **Saturday 1 August 2026** (handover same day)
House: 3 bedroom, 2 bathroom

---

## Timeline Overview

### Week 1 — 4 Weeks Out (1 July → 6 July)
- [ ] Confirm when current occupants vacate new place
- [ ] Decide: hire movers vs self-move (get 2–3 quotes if hiring)
- [ ] Set moving budget
- [ ] Collect packing supplies — boxes, tape, bubble wrap, markers, labels, furniture pads
  - Estimate: 80–100 boxes for a 3-bed house
- [ ] Start a "purge pass" — go room by room and decide: keep / donate / sell / trash
- [ ] List large items that need special handling (TVs, appliances, furniture)
- [ ] Notify: employer, bank, insurance, subscriptions, Home Affairs / post office (address change)
- See [[Moving/Week 1 Action Plan|Week 1 Action Plan]] for full detail

### Week 2 — 3 Weeks Out (7 July → 13 July)
- [ ] Begin packing non-essential / rarely used rooms first (see room order below)
- [ ] Sort and label all boxes: **Room | Contents | Fragile?**
- [ ] Take photos of all cable setups (TV, PC, home lab, etc.) before disassembly
- [ ] Arrange utility connections at new place (internet, electricity)
- [ ] Arrange utility disconnections / transfer at current place

### Week 3 — 2 Weeks Out (14 July → 20 July)
- [ ] Pack secondary bedrooms
- [ ] Pack garage / storage areas
- [ ] Pack non-daily-use kitchen items
- [ ] Confirm move date/time with movers or helpers
- [ ] Start consuming/clearing fridge and freezer contents

### Week 4 — 1 Week Out (21 July → 27 July)
- [ ] Pack main bedroom (keep 3–4 days of clothes aside)
- [ ] Pack bathrooms (keep toiletries kit aside)
- [ ] Pack remaining kitchen — leave out 1 pot, 1 pan, basic cutlery
- [ ] Disassemble furniture that needs it — bag all screws/bolts and tape to the piece
- [ ] Re-hang all blinds and replace all removed fixtures
- [ ] Fill all holes, sand and touch up paint

### Final Push (Thu 30 – Fri 31 July) — Take These Days Off
- [ ] Thursday 30: final packing, wall touch-ups, blinds and fixtures done
- [ ] Friday 31: defrost fridge and deep freeze, final walkthrough, pack First Night Box, confirm movers

### Moving Day — Saturday 1 August
- [ ] Walk movers through the house and label system
- [ ] Keep First Night Box with you (not on the truck)
- [ ] Cat in safe room until everything is loaded
- [ ] Do final walkthrough before handing over keys — check every room, cupboard, outside
- [ ] Hand over keys

---

## Packing Order (Least Disruptive → Most)

1. Garage
2. Outside Porch
3. Spare bedroom / guest room
4. Study / Home Lab
5. Living room / lounge
6. Main Gaming Area
7. Main bedroom
8. Kitchen
9. Bathrooms

---

## Room-by-Room Breakdown

### Main Gaming Area
**Computers / Tech:**
- [ ] Photograph all cable setups (PC rear, monitors, peripherals) before touching anything
- [ ] Shut down cleanly, disconnect and label all cables
- [ ] PC towers — wrap in moving blanket, transport upright, keep away from heavy items
- [ ] Monitors — wrap screens in bubble wrap or moving blanket, transport upright never flat, original boxes ideal
- [ ] Peripherals (keyboards, mice, headsets, controllers) — bundle cables per device with zip-ties or velcro, pack together in a labelled box

**Furniture:**
- [ ] Desks — measure against new room/doorways, disassemble if needed, bag all hardware
- [ ] Cat post — comes with us, wrap base to protect flooring

**Shelf:**
- [ ] LEGOs — if built sets, wrap individually in bubble wrap or cling wrap to hold pieces, pack in sturdy boxes, mark FRAGILE / THIS SIDE UP
- [ ] If loose/unbuilt sets, keep in original boxes or zip-lock bags labelled per set

**General:**
- [ ] This is a high-value room — pack PCs and monitors yourself rather than leaving to movers
- [ ] Consider transporting PCs and monitors in your own car

---

### Main Bedroom
**Furniture (coming with us):**
- [ ] King size bed — disassemble frame, bag all bolts/screws and tape to frame, headboard wrap in moving blanket
- [ ] Two side tables — small, straightforward, empty drawers before moving
- [ ] Dressing table — empty all drawers first, wrap mirror if attached, bag hardware if disassembling legs

**Sell / replace:**
- [ ] Three fans — sell or donate all three, buy one new fan at new place instead

**Fixed cupboards (staying, empty out):**
- [ ] Do a clothing purge pass — old/unwanted clothes into donation bags before packing
- [ ] Out of season / rarely worn clothes → pack first (3 weeks out)
- [ ] Daily wear → pack last, keep a week's worth accessible until moving day
- [ ] Use bin bags for clothes on hangers — leave on hanger, bag from the bottom

**Dressing table drawers:**
- [ ] Sort contents before packing — good opportunity to purge
- [ ] Pack drawer contents into clearly labelled boxes ("Dressing Table - Drawer 1" etc.)
- [ ] If drawers can be removed and transported in place, cling-wrap them shut

**Ensuite Bathroom:**
- [ ] See Bathroom section below — pack last, keep toiletries accessible until moving day
- [ ] Check for any fixtures or fittings removed that need to go back (see House Handover)

### Spare Bedroom
**Furniture:**
- [ ] Queen size bed (coming with us) — disassemble frame, bag all bolts/screws and tape to headboard
- [ ] Fixed cupboard — stays, empty it out completely before moving day

**Cupboard contents (to pack):**
- [ ] Towels — use these to wrap fragile items in kitchen/study boxes, pack remainder in bags/boxes
- [ ] Blankets and linen — large bags or boxes, good filler material around fragile items
- [ ] Loose boxes and miscellaneous items — purge pass, keep only what's needed
- [ ] Blinds storage — see House Handover section below, blinds need to go back up before leaving

**Other items (all coming with us):**
- [ ] Plushies — pack in large bags or boxes, lightweight so good for filling space in bigger boxes
- [ ] Old monitor — wrap screen in bubble wrap or blanket, pack upright, label FRAGILE
- [ ] Funkos — already boxed, just confirm boxes are sealed and labelled, stack carefully
- [ ] Chest with packed books — heavy, confirm chest can be moved as-is or transfer books to smaller boxes for transport
- [ ] Loose shelf — disassemble if possible, wrap shelving boards together, bag any brackets/screws

**General:**
- [ ] Can be fully packed 2–3 weeks out once blinds are re-hung and cupboard is cleared
- [ ] Mostly already packed (Funkos, books) — good head start, just needs consolidating

### Study / Home Lab
> This is the most complex room — plan extra time and do it methodically.

**Network / Fibre — Do Early (Week 2)**
- [ ] Unmount router and switch from table early — place on floor or shelf to free up the table for moving, keep everything plugged in and running
- [ ] Remove ethernet trunking from walls around the house early — can live with exposed cables for a few weeks, no rush to tidy
- [ ] Photograph all cable runs and port assignments before touching anything
- [ ] Router, switch, ONT, and all patch cables — pack together in a clearly labelled bag/box on moving day, transport in your car not the truck
- [ ] Arrange ISP reconnection at new address before or on moving day

**Home Assistant — Disassemble Early (Week 2)**
- [ ] Home Assistant setup can come down early — stability testing will restart fresh at the new place
- [ ] Shut down cleanly, pack HA hardware (SBC, SD card, power supply) together in a labelled bag
- [ ] No need to keep it running during the move

**Electronics Cupboard (3 shelves)**
- [ ] Do a purge pass first — bin broken parts, donate unused gear
- [ ] Photograph shelf layout before clearing
- [ ] Pack per shelf into labelled boxes: "Study - Electronics Cupboard - Shelf 1/2/3"
- [ ] Small components: sort into zip-lock bags, label contents, pack into a single box

**Large Cupboard / Box Storage Area**
- [ ] Purge pass — flatten and recycle boxes not worth keeping
- [ ] Identify any useful boxes to repurpose for packing other rooms

**Desk / Table Area (3 tables)**
- [ ] Table 1 — 3D Printer
  - [ ] Remove print bed, secure gantry/extruder for transport (consult printer manual)
  - [ ] Pack filament separately (see filament shelf below)
  - [ ] Box or wrap printer securely — it's delicate
- [ ] Table 1 — Network switches
  - [ ] Label all cables before disconnecting (numbered tape or sticky labels)
  - [ ] Pack switches with their power bricks together
- [ ] Table 1 — Raspberry Pi + LattePanda Delta
  - [ ] Shutdown cleanly, photograph cable setups
  - [ ] Pack SBCs with their power supplies and SD cards in a padded box
- [ ] Remaining tables — disassemble if possible, label all hardware

**Book Cupboard (coming with us)**
- [ ] Soldering station + irons — pack iron tips safely, remove solder from dispensers if possible
- [ ] Electronics components (resistors, caps, modules, etc.) — keep in existing organiser trays if possible, wrap trays in cling wrap to keep contents in place
- [ ] Books — small boxes only (heavy), pack upright like in a bookshelf
- [ ] Purge pass: old project parts, broken components, things that haven't been touched in a year

**3D Filament Shelf**
- [ ] Seal all open filament spools in zip-lock bags or vacuum bags to prevent moisture absorption during the move
- [ ] Pack spools flat or on-edge — avoid stacking too many (weight crushes lower spools)
- [ ] Label box: "Study - Filament - FRAGILE / KEEP DRY"

**General**
- [ ] Do a full purge pass before any packing starts — this room will have the most "decision fatigue" items
- [ ] Pack non-essential gear 3 weeks out, active home lab gear in the final week
- [ ] Keep a "daily use" pile (laptop, chargers, mouse/keyboard) separate until moving day

### Bathroom 1 & Ensuite (Bathroom 2)
> Pack last — day before or morning of move.

- [ ] Purge pass — discard old/expired toiletries, half-empty products not worth moving
- [ ] Pack each person's toiletries into their own bag/box — keep accessible until moving day
- [ ] Decant or secure liquid bottles upright in a zip-lock bag in case of leaks
- [ ] Towels — use to wrap fragile items in other rooms first, then box remaining
- [ ] Check for any wall fixtures removed that need to go back (towel rails, hooks, toilet roll holders — see House Handover)

### Kitchen
**Large appliances:**
- [ ] Fridge — defrost and clean 24 hrs before move, disconnect, leave door open to air out, transport upright, do not plug in for at least 4 hrs after moving
- [ ] Dishwasher — disconnect water inlet and drain hose, run a clean cycle before disconnecting, tape door shut for transport, transport upright
- [ ] Microwave — wrap in moving blanket, keep door secured with tape

**Sell / give away:**
- [ ] Soda machine — rarely used, list for sale or give away before moving week

**Small appliances (pack mid — 2 weeks out):**
- [ ] Pressure cooker — remove and pack insert/seal separately
- [ ] Toaster — shake out crumbs, wrap and box
- [ ] Snackwich maker — clean plates before packing, wrap in paper or tea towel
- [ ] Air fryer — remove basket, wrap both separately

**Crockery / utensils (pack mid):**
- [ ] Plates and bowls — wrap each in paper or tea towel, pack flat in small boxes, layer with padding between stacks, FRAGILE
- [ ] Cups and mugs — wrap individually, pack upright, FRAGILE
- [ ] Eating utensils — bundle in zip-lock bags or rubber band, box together
- [ ] Tupperware — nest lids separately, pack lids in a bag inside the same box

**Food / pantry (pack last — final week):**
- [ ] Consume or donate perishables before moving week
- [ ] Dry goods — seal open packets in zip-lock bags before boxing
- [ ] Condiments / sauces — upright in a box lined with a bin bag in case of leaks

**General:**
- [ ] Use towels and tea towels to wrap fragile items — saves on bubble wrap
- [ ] Mark all kitchen boxes **FRAGILE / THIS SIDE UP**
- [ ] Pack one "last night / first morning" kit: kettle, mugs, teaspoon, coffee/tea, snacks — goes in car not truck

### Living Room / Lounge
**Electronics:**
- [ ] TV — photograph wall/stand mount setup, wrap screen in moving blanket or bubble wrap, transport upright never flat, original box ideal else double-box
- [ ] TV stand — check if disassembles, bag all hardware

**Seating:**
- [ ] Couch set — check if sections detach, wrap in moving blankets, measure against new place doorways if it's a large set

**Storage / Display:**
- [ ] Bookcase 1 — books into small boxes (heavy fast), label by genre/series if order matters
- [ ] Bookcase 2 — board games with custom inserts so loose pieces aren't a concern, pack upright (never flat or stacked on their faces), ensure boxes don't crush each other — use packing paper or linen as padding between games in the box, mark FRAGILE
- [ ] Disassemble bookcases for transport, bag all screws/bolts

**Other:**
- [ ] Cat feeding table — clean, disassemble if possible
- [ ] Cat food, bowls, accessories — pack day of or keep accessible until last

**General:**
- [ ] Purge pass on books and board games — good opportunity to donate anything unwanted before boxing them all up
- [ ] Board games with lots of loose pieces: rubber band or cling-wrap boxes shut before packing

### Garage
**Large appliances (moving with us):**
- [ ] Deep freeze — defrost and clean 24 hrs before move, transport upright, do not plug in for 4 hrs after
- [ ] Tumble dryer — disconnect drum strap/secure drum if needed, keep power cord accessible

**Sell before move:**
- [ ] Standing light (try sell — could repurpose in study at new place if no buyer)

**Trash / donate:**
- [ ] Old microwave (not used, not worth moving)
- [ ] Any boxes not suitable for packing

**Boxes / packing materials:**
- [ ] Do a preliminary purge pass — sort usable boxes from junk
- [ ] Keep sturdy double-walled boxes for packing, break down and dump the rest
- [ ] Label useful boxes by size for redistribution to other rooms

**General:**
- [ ] Tools: wrap sharp items safely, pack in small/medium boxes
- [ ] Label all boxes: "Garage - [category]"

### Outside Porch
**Coming with us:**
- [ ] Chair and table set — clean before packing, wrap/stack for transport

**Sell / give away:**
- [ ] Porch bed — list for sale or give away early (bulky, don't want it sitting around)
- [ ] Braai — sell or give away, not coming with us

**General:**
- [ ] Sort through loose items — clean what's worth keeping, trash the rest
- [ ] This area will likely need a dedicated cleaning session before it can be properly assessed
- [ ] Do porch purge early — outdoor items can be sold/collected before the main moving week

---

## First Night Box

Pack a clearly labelled box / bag that rides with you, not the truck:
- [ ] Toiletries (toothbrush, soap, deodorant)
- [ ] 2 days of clothing per person
- [ ] Bedding for first night (1 set of sheets, pillows)
- [ ] Phone chargers + laptop
- [ ] Basic snacks / coffee / kettle
- [ ] Toilet paper
- [ ] Important documents (IDs, lease/title deeds, insurance)
- [ ] Medications
- [ ] Kids / pets essentials

---

## Packing Tips

- **Label every side of boxes** — not just the top
- **Small boxes for heavy items** (books, tools), large boxes for light/bulky (linen, pillows)
- **Fill boxes completely** — half-full boxes collapse under weight
- **Colour-code by room** — coloured tape or sticker dots per room makes unloading faster
- **Don't mix rooms in one box**
- **Photograph valuables** before packing for insurance purposes
- **Clothing hack**: leave clothes on hangers, cover with a bin bag from the bottom — instant wardrobe box

---

## Supplies Checklist

- [ ] Boxes (small, medium, large) — ~80–100 total
- [ ] Packing tape + dispenser (×2)
- [ ] Bubble wrap / packing paper
- [ ] Permanent markers (×4)
- [ ] Labels / stickers (or coloured tape per room)
- [ ] Furniture pads / moving blankets
- [ ] Zip-lock bags (for hardware, screws)
- [ ] Bin bags (large, for clothing on hangers)
- [ ] Stretch wrap / plastic wrap (for drawers, furniture)
- [ ] Box cutter

---

## Pets (Cat)

> Cats are highly territorial and stress easily during moves. Plan around them deliberately — the steps below make a significant difference.

### Weeks Before (Start Now)
- [ ] Leave the cat carrier out in a familiar spot so the cat gets comfortable with it — put a blanket or toy inside
- [ ] Consider buying a **Feliway diffuser** — plug it in now at home to start calming effect, then move it to the new place a week before/after move
- [ ] Keep furniture in place as long as possible — rearranging the home stresses cats before you've even moved
- [ ] Maintain feeding schedule and litter box location throughout packing weeks
- [ ] Stock up on cat food, litter, and supplies to last through moving week
- [ ] Fit cat with an **ID collar** with your new address on it before moving day (in case of escape)
- [ ] Update microchip registry with new address

### Moving Day
- [ ] Designate a **safe room** (e.g. bathroom or spare room) — set it up with: carrier, bed, familiar blanket, food, water, litter tray, and scratching post
- [ ] Confine cat to safe room before movers arrive and before any doors are left open
- [ ] Put a clear note on the door: **"Cat inside — do not open"**
- [ ] Do not feed cat 2–3 hours before travel to prevent travel sickness
- [ ] Once everything is loaded, place cat in carrier and transport in your **own car** — never the moving truck
- [ ] Cover the carrier with a light blanket to reduce visual stress during the journey
- [ ] Bring in the car: food, water bowl, litter tray, familiar blanket/toy

### At the New Place
- [ ] Set up a **safe room first** before letting the cat out of the carrier — same setup as the safe room at the old place (familiar smells are key)
- [ ] Place the carrier in the safe room next to a hiding spot and open it — let the cat come out in their own time, don't force it
- [ ] Keep the cat in this one room for the first 1–2 days while you unpack and settle
- [ ] Gradually open up one room at a time over the following days as the cat grows confident
- [ ] **Scent technique**: rub a soft cloth gently around the cat's cheeks and head, then rub the cloth on doorframes, walls, and furniture at cat height around the new home — spreads their scent and makes the space feel safe
- [ ] Keep all windows, doors, and screens closed and secure for the first few weeks

### Settling In (Weeks After)
- [ ] Keep cat **fully indoors for at least 3 weeks** — this is when cats are most likely to bolt back to the old address
- [ ] Stick to the same feeding schedule, same food, same litter brand — don't change anything else while they adjust
- [ ] Give extra attention, play, and treats to build positive associations with the new home
- [ ] After 3+ weeks, if the cat seems settled, begin supervised outdoor access
- [ ] Update vet records with new address

---

## Should You Take Days Off?

**Yes — strongly recommended.** Here's a suggested approach:

- **Thursday 30 July** — Take off: final packing push, touch up walls/holes, re-hang blinds and fixtures
- **Friday 31 July** — Take off: defrost fridge and deep freeze, clean both, final walkthrough of every room, pack First Night Box, confirm movers
- **Saturday 1 August** — Moving day and handover

This gives you Thu–Fri to close everything out calmly before the Saturday move. Trying to finish on weekends only and scrambling on the day is the most common source of moving stress.

> If you can only take 1 day: take Friday 31 July.

---

## House Handover / Restoration

> These need to be done before handing back keys — ideally in the final week, not on moving day itself.

**Blinds**
- [ ] Retrieve blinds from spare bedroom storage
- [ ] Re-hang all blinds in their original positions room by room
- [ ] Do a walkthrough to confirm every window that had a blind has one back up

**Fixtures**
- [ ] Walk every room and identify fixtures that were removed (hooks, brackets, curtain rails, etc.)
- [ ] Reattach all removed fixtures before handing over
- [ ] Keep a bag of the relevant screws/wall plugs together so they're accessible for this task

**Holes and Wear**
- [ ] Do a full wall inspection — note all holes (picture hooks, rawl plugs, cable clips, etc.)
- [ ] Fill all holes with polyfilla / wall filler
- [ ] Sand smooth once dry, touch up paint if needed
- [ ] Do this early enough in the final week so paint/filler has time to dry before inspection

**Final Walkthrough (Day of Handover)**
- [ ] Every room checked for fixtures, blinds, damage
- [ ] All walls filled and presentable
- [ ] All cupboards and storage emptied
- [ ] All keys accounted for (front door, garage, gate, postbox, etc.)

---

## Notes / Decisions

- [ ] Confirm new address
- [ ] Measure doorways at new place vs large furniture
- [ ] Decide what doesn't make the move (sell/donate before moving date)

---

## Links
- [[Moving/4-Week Master Timeline|4-Week Master Timeline]]
- [[Moving/Week 1 Action Plan|Week 1 Action Plan]]
- [[index]]
